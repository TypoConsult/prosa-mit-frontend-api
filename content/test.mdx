...
# PROSA CRM API Documentation

## Overview

This API provides endpoints for member registration and retrieval through PROSA's web form system. The API uses HTTP POST requests and returns JSON responses.
...
> ⚠️ **Important:** You cannot determine if account creation was successful by only checking the `SubmitWebformular` response.
